  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
                      ...
                      <td className="px-3 py-2.5">
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="p-1.5 rounded-md text-ink3 hover:text-accent hover:bg-accent/10 transition-all"
                            title="Edit order">
                            <Pencil size={13}/>
                          </button>
                          <button
                            onClick={() => setDeleteOrder(o)}
                            className="p-1.5 rounded-md text-ink3 hover:text-danger hover:bg-danger/10 transition-all"
                            title="Delete order">
                            <Trash2 size={13}/>
                          </button>
                        </div>
      ...
      <Modal open={!!editOrder} onClose={() => setEditOrder(null)}
        title={`✏️ Edit Order ${editOrder ? formatOrderNumber(editOrder.order_number) : ''}`}
        size="md"
        footer={
          <>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <Select label="Payment" value={editOrder.payment_mode} onChange={e => setEditOrder(p => p ? {...p, payment_mode: e.target.value as PaymentMode} : p)}>
                ...
                <option value="zomato_pay">Zomato Pay</option>
              </Select>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Input label="Total Amount Rs." type="number" value={editOrder.total} onChange={e => setEditOrder(p => p ? {...p, total: Number(e.target.value)} : p)}/>
            ...
            <Input label="Notes" value={editOrder.notes ?? ''} onChange={e => setEditOrder(p => p ? {...p, notes: e.target.value} : p)} placeholder="Optional notes…"/>

            {/* Items — read only, just show */}
      ...
      <Modal open={!!deleteOrder} onClose={() => setDeleteOrder(null)}
        title="🗑️ Delete Order"
        footer={
          <>
            <Button variant="ghost" onClick={() => setDeleteOrder(null)}>Cancel</Button>
        ...
        {deleteOrder && (
          <div>
            <p className="text-[14px] text-ink mb-3">
              Are you sure you want to delete <strong>{formatOrderNumber(deleteOrder.order_number)}</strong>?
            </p>
            <div className="bg-surface2 border border-border rounded-lg p-3 text-[12px] space-y-1">
              ...
              <div><span className="text-ink3">Amount:</span> <span className="font-medium text-accent">{formatCurrency(deleteOrder.total)}</span></div>
            ...
            <div className="mt-3 bg-danger/8 border border-danger/20 rounded-lg px-3 py-2 text-[12px] text-danger">
              ⚠️ This will permanently delete the order and all its items. This cannot be undone.
            </div>
          </div>
        )}
      </Modal>
