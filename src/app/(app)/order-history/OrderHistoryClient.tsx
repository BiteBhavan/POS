  ...
  async function handleSaveEdit() {
    ...
    try {
      const { error } = await supabase.from('orders').update({
        ...
        payment_mode: editOrder.payment_mode,
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
          ...
          {filtered.map(o => (
            <div key={o.id} className="bg-white border border-border rounded-lg p-3 shadow-sm">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <span className="font-bold text-accent text-[13px]">{formatOrderNumber(o.order_number)}</span>
                ...
                <div className="flex items-center gap-1">
                  ...
                  {isOwner && (
                    <>
                      <button onClick={() => setEditOrder({...o})} className="p-1.5 rounded text-ink3 hover:text-accent hover:bg-accent/10 transition-all"><Pencil size={13}/></button>
                      <button onClick={() => setDeleteOrder(o)} className="p-1.5 rounded text-ink3 hover:text-danger hover:bg-danger/10 transition-all"><Trash2 size={13}/></button>
                    </>
                  )}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-1 text-[12px] mb-2">
                ...
                <div><span className="text-ink3">Customer: </span><span className="font-medium">{o.customer_name ?? '—'}</span></div>
              ...
              <div className="text-[11px] text-ink3 border-t border-border pt-2">
                {o.order_items?.map(i => (
                  <span key={i.id} className="mr-2">{i.quantity}× {i.item_name}{i.note ? ` [${i.note}]` : ''}</span>
                ))}
              </div>
            </div>
          ))}
          {filtered.length === 0 && <div className="text-center py-10 text-ink3 text-sm">No orders found</div>}
        </div>
        ...
        <div className="hidden md:block bg-white border border-border rounded-lg overflow-hidden shadow-sm">
          <div className="overflow-x-auto">
            <table className="w-full border-collapse min-w-[800px]">
              <thead>
                <tr>
                  {['Order #','Date','Source','Address','Customer','Items','Pay','Amount','Status', isOwner ? 'Actions' : ''].map(h => (
              ...
              <tbody>
                ...
                {filtered.length === 0 && (
                  <tr><td colSpan={isOwner ? 10 : 9} className="text-center py-10 text-ink3 text-sm">No orders found</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      <Modal open={!!editOrder} onClose={() => setEditOrder(null)}
        title={'Edit Order ' + (editOrder ? formatOrderNumber(editOrder.order_number) : '')}
        footer={<><Button variant="ghost" onClick={() => setEditOrder(null)}>Cancel</Button><Button variant="primary" loading={saving} onClick={handleSaveEdit}>Save Changes</Button></>}>
        {editOrder && (
          <div className="space-y-3">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              ...
              <Select label="Payment" value={editOrder.payment_mode} onChange={e => setEditOrder(p => p ? {...p, payment_mode: e.target.value as PaymentMode} : p)}>
                ...
                <option value="card">Card</option><option value="zomato_pay">Zomato Pay</option>
              </Select>
            ...
            <Select label="Status" value={editOrder.status} onChange={e => setEditOrder(p => p ? {...p, status: e.target.value as OrderStatus} : p)}>
              ...
              <option value="ready">Ready</option><option value="out_for_delivery">Out for Delivery</option>
            ...
            {editOrder.order_items && editOrder.order_items.length > 0 && (
              <div>
                <label className="text-[10px] text-ink2 uppercase tracking-wider font-semibold block mb-1.5">Items (read-only)</label>
                <div className="bg-surface2 border border-border rounded-md px-3 py-2 space-y-1">
                  {editOrder.order_items.map(i => (
                    <div key={i.id} className="text-[12px] text-ink2 flex justify-between">
                      <span>{i.quantity}× {i.item_name}{i.note ? <span className="text-orange-600 italic"> · {i.note}</span> : ''}</span>
                      <span className="text-ink3">{formatCurrency(i.total_price || 0)}</span>
                    </div>
                  ))}
                </div>
                <p className="text-[11px] text-ink3 mt-1">To change items — delete this order and create a new one.</p>
              </div>
            )}
          </div>
        )}
        ...
        {deleteOrder && (
          <div>
            ...
            <div className="bg-surface2 border border-border rounded-lg p-3 text-[12px] space-y-1 mb-3">
              ...
              <div><span className="text-ink3">Address: </span><span className="font-medium">{deleteOrder.address_short_code ?? '—'}</span></div>
            ...
            <div className="bg-danger/8 border border-danger/20 rounded-lg px-3 py-2 text-[12px] text-danger">
              ⚠️ This permanently deletes the order and all items. Cannot be undone.
            </div>
          </div>
        )}
      </Modal>
